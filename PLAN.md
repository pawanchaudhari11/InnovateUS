...
`index.html` / `style.css`: nav + footer styled to the brand kit, registration card expanded to match the confirmed schema — first/last name, email, country (+ conditional state), gov-org question with 4 verbatim answer options (+ conditional gov level), a "Selected Event Series" multi-select checklist (see Phase 0), newsletter checkbox, submit button. Other real-site elements (nav links, any SSO-style buttons) are visual only, per your note — not wired up.

Design pass refined against the real site's actual CSS (fetched from `innovate-us.org/register`'s Nuxt bundle) rather than guesswork: real fonts (DM Serif Display/Text, Libre Franklin), real footer copy ("Want to be a part of our community of innovators? We'd love to keep in touch.", "Join Our Mailing List", real CC license text), real footer link/column structure, real button/input treatment. Card layout, navbar arrow icon, and spacing were iterated live with you.
